...
class Evatix_Artists_Adminhtml_ArtistsController extends Mage_Adminhtml_Controller_action {
    ...
    public function imagesAction() {
        $imagePanel .="<div class=\"entry-edit\">
                        <div class=\"entry-edit-head\">
                            <h4 class=\"icon-head head-edit-form fieldset-legend\">Images (* First Image will be count as Main Image of Artist)</h4>
                        </div>
    ...
    private function uploadImage($artistId = 0) {
        if ($_FILES && $artistId) {
            try {
                ...
                //$path = Mage::getBaseDir('media') . DS . 'artists' . DS . $artistId. DS;
                $tableName = Mage::getSingleton('core/resource')->getTableName('artists_image');
                ...
                try {
                    ...
                    $query = 'DELETE FROM ' . $tableName . ' WHERE artists_id='.$artistId.' AND unique_index NOT IN(' . $indexList . ')';
                ...
                foreach ($_FILES as $key => $value) {
                    $fname = str_replace(' ', '_', $value['name']);
                    if ($fname) {
                        ...
                        $fname = strtotime(date('Y-m-d H:i:s')).'_'.$artistId.'_'.$imageIndex.$fname;
                        $uploader->save($path, $fname);
                        try {
    ...
    /**
     * Save Data in Category And Artist Relation Table
     * @param   int     $artistid
     * @param   array   $categoryIds
     */
    private function saveCatArtist($artistid = 0, $categoryIds = array()) {
        try {
            $coreResource = Mage::getSingleton('core/resource');
            $tableName = $coreResource->getTableName('artists_category_rel');
            $writeConn = $coreResource->getConnection('core_write');

            $dquery = 'DELETE FROM ' . $tableName . ' WHERE artists_id=' . $artistid;
            $writeConn->query($dquery);

            $query = "INSERT INTO " . $tableName . " (artists_id,category_id)
                      VALUES (:artists_id,:catid)";
            if ($artistid) {
                $data['artists_id'] = intval($artistid);
                foreach ($categoryIds as $catid) {
                    $data['catid'] = intval($catid);
                    $writeConn->query($query, $data);
                }
            }
        } catch (Exception $ex) {
            Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
        ...
        if ($data = $this->getRequest()->getPost()) {
            if ($this->_validatePostData($data)) {
                try {
                    ...
                    if (isset($data['thumbnail']['delete']) && $data['thumbnail']['delete']) {
                        unset($data['thumbnail']);
                        $data['thumbnail'] = '';
                    } else if ($_FILES && isset($_FILES['thumbnail']['name'])) {
                        $path = Mage::getBaseDir('media') . DS . 'artists' . DS . 'thumb' . DS;
                        $fname = str_replace(' ', '_', $_FILES['thumbnail']['name']);
                        if ($fname) {
                            ...
                            /** save Image * */
                            $uploader->save($path, $fname);
                            $data['thumbnail'] = 'artists/thumb/'.$fname;
                        } else {
                            unset($data['thumbnail']);
                        }
                    ...
                    $this->saveCatArtist($artistId, $category);
                    /** Upload Artist Photo * */
                    /** Send Email * */
