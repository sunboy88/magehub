...
-- DROP TABLE IF EXISTS {$this->getTable('artists_category_rel')};
CREATE TABLE {$this->getTable('artists_category_rel')} (
    `artists_id` int(11) unsigned  NOT NULL,
    `category_id` int(11) unsigned  NOT NULL,
    PRIMARY KEY (`artists_id`, `category_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8;

ALTER TABLE `artists_category_rel`
  ADD CONSTRAINT `artists_category_rel_ibfk_1` FOREIGN KEY (`category_id`) REFERENCES `artists_category` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `FK_artists_category_rel_artist_id_artists` FOREIGN KEY (`artists_id`) REFERENCES `artists` (`artists_id`) ON DELETE CASCADE ON UPDATE CASCADE;
