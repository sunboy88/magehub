...
jQuery(function($) {
	...
	function _qsJnit(){
		...
		var _qsHref = "<div class=\"abc\" style=\"visibility:hidden;position:absolute;top:0;left:0\"><a id=\"md_quickview_handler\" href=\"#\" ><img class=\"image\" style=\"display:block;\" alt=\"quickview\" src=\""+EM.Quickview.QS_IMG+"\" /></a></div>";
		...
		$.each(listprod, function(index, value) { 
			...
			//get reload url
			//console.log($(value).find('.test').html());
			var content = "";
			var name = $(value).find('.product-name').html();
			//var price = 
			content = "<div id=\"content-quickview\">" + name + "</div>";
			//_qsHref += name + "</div>";
			//console.log(_qsHref);
			...
			$(selectorObj.imgClass, this).bind('mouseover', function() {
				var o = $(this).offset();
				$('.abc').css({
						'top': o.top+($(this).height() - qsHandlerImg.height())/2 + 30 +'px',
						'left': o.left+($(this).width() - qsHandlerImg.width())/2 - 20 +'px',
						'visibility': 'visible'
					});
			...
			$(value).bind('mouseout', function() {
				...
				$('.abc #content-quickview').remove();
			});
		});

		//fix bug image disapper when hover
		$('#md_quickview_handler').bind('mouseover', function() {
				$('.abc #content-quickview').css('display','block');
				$(this).show();
			})
			.bind('click', function() {
				$(this).hide();
